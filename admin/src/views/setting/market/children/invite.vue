<template>
  <div class="app-container">
    <el-form ref="form" v-loading="loading" :model="form" :rules="rules" label-width="150px">
      <el-form-item label="邀请人获得" prop="type">
      ...
      <section v-if="form.type.indexOf('points') >= 0">
        ...
        <el-form-item label="每月积分奖励上限" prop="points_limit">
          <el-input-number v-model="form.points_limit" :min="0" label="每月积分奖励上限" />
          <div style="padding-top: 10px;">
            <el-alert :closable="false" type="info">
              <div slot="title" style="line-height: 30px;">每个自然月用户最多能获取的奖励积分，0为无上限。</div>
            </el-alert>
          </div>
        </el-form-item>
      </section>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      rules: {
        points: [{ required: true, message: '请输入邀请人获得积分', trigger: 'blur' }],
        points_limit: [{ required: true, message: '请输入每月积分奖励上限', trigger: 'blur' }],
    ...
    get('market.invite')
      .then(res => {
        if (res.data) {
          ...
          this.form.points_limit = res.data.points_limit
